...
- `security/JwtService` signs and verifies HMAC tokens (`jwt.secret` property - a committed dev value for the `h2` profile, read from the `JWT_SECRET` env var for `mysql`/`railway`).
...
**Error handling.** `shared/exception/BusinessException` + `shared/exception/GlobalExceptionHandler` (`@RestControllerAdvice`) are the only error path - throw `BusinessException` for any domain/validation failure and it becomes a 400 with a JSON body (`{timestamp, message}`); anything else falls through to a generic 500. There's no distinction yet between "not found," "unauthorized," and "invalid input" - they're all mapped to 400. If you need proper status codes for specific failures, extend `GlobalExceptionHandler` with a more specific exception hierarchy rather than stringly-typed messages.

**Config profiles.** `spring.profiles.active` (set in `application.properties`, default `h2`) selects `application-{h2,mysql,railway}.properties`. `h2`/dev has an inline `jwt.secret` and in-memory DB; `mysql`/`railway` expect `JWT_SECRET` plus DB credentials from environment variables - don't hardcode secrets into those two files.
